...
def extract_tables(sql: str) -> dict[str, str]:
    """
    Returns alias -> full_table_name.
    Example:
    FROM INVENTORY.PURCHASEORDER po
    returns {"PO": "INVENTORY.PURCHASEORDER"}
    """
    tables = {}
...
def validate_sql_columns(sql: str) -> bool:
    ...
    # Validate qualified columns like po.ORDERNO or INVENTORY.PURCHASEORDER.ORDERNO
    qualified_columns = re.findall(r"\b([A-Z0-9_]+)\.([A-Z0-9_]+)\b", sql_upper)

    for prefix, column in qualified_columns:
        # Skip schema.table references
    ...
    # Validate simple selected columns only for single-table queries
